document.addEventListener("DOMContentLoaded", function() {
    ...
    // Añade la funcionalidad para guardar y enviar el premio a la base de datos
    var saveAndSendButton = document.querySelector(".save-and-send");
    var prizeInput = document.getElementById("prizeInput");

    saveAndSendButton.addEventListener("click", function() {
        var confirmAction = confirm("¿Estás seguro de que deseas enviar el formulario?");
        if (confirmAction) {
            var prize = stopRotateWheel();
            prizeInput.value = prize;

            // Envía el formulario usando Fetch API
            var formData = new FormData(document.getElementById("resultForm"));
            fetch('../src/guardar_priza.php', {
                method: 'POST',
                body: formData
            }).then(response => response.text())
            .then(data => {
                console.log(data); // Puedes manejar la respuesta del servidor aquí
                // Redireccionar a client/inicio.html
